...
export async function onRequest({ request, env }) {
  ...
  try {
    let apiKey, page = "1", genre, country;
    ...
    // Build target URL based on filters
    let targetUrl;
    if (country) {
      targetUrl = `${BASE}/country/${country}/`;
    } else if (genre) {
      targetUrl = `${BASE}/genre/${genre}/`;
    } else {
      targetUrl = `${BASE}/`;
    }
