...
// .wrangler/tmp/pages-mK4XoS/functionsWorker-0.2578679781254132.mjs
...
var init_esm = __esm({
  "../node_modules/domelementtype/lib/esm/index.js"() {
    init_functionsRoutes_0_11914055566704662();
...
async function onRequestGet({ request }) {
  ...
  try {
    ...
    const videoData = extractVideoData(html3);
    return new Response(JSON.stringify(videoData, html3), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*"
...
function extractVideoData(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const sources = [];
  $2("video#censor-player source").each((_, element) => {
    const src = $2(element).attr("src");
    let label = $2(element).attr("label");
    if (label) {
  ...
  if (sources.length > 0) {
    ...
    return { error: "No MP4 links found", html: html3 };
  }
}
__name(extractVideoData, "extractVideoData");
var init_getVideo = __esm({
  "api/desipin/getVideo.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet, "onRequestGet");
    __name2(extractVideoData, "extractVideoData");
  }
});
async function onRequestGet2({ request }) {
  const { searchParams } = new URL(request.url);
  const page = searchParams.get("p") || 0;
  const targetURL = `https://topdesix.com/page/${page}/`;
  try {
    const headers = {
      Accept: "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
      "Accept-Language": "en-US,en;q=0.9",
      "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
      Cookie: "_ym_uid=1731821805711588892; _ym_d=1731821805; is_18=true"
    };
    ...
    const html3 = await response.text();
    const videos = extractVideoData2(html3);
...
function extractVideoData2(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const videoElements = $2(".video-block");
  return videoElements.map((_, video) => {
    const element = $2(video);
    const title = element.find(".infos .title").text().trim();
    const time = element.find(".video-datas .views-number").first().text().trim();
    const preview = element.find(".thumb-img").attr("src");
    const href = element.find(".thumb").attr("href");
    const videoUrl = element.find(".thumb-video").attr("src");
    return { title, time, preview, href, videoUrl };
  }).get();
}
__name(extractVideoData2, "extractVideoData2");
var init_scrapeDESI = __esm({
  "api/desipin/scrapeDESI.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet2, "onRequestGet");
    __name2(extractVideoData2, "extractVideoData");
  }
});
async function onRequestGet3({ request }) {
  const { searchParams } = new URL(request.url);
  const page = searchParams.get("p") || 0;
  const targetURL = `https://erothots1.com/free-porn?p=${page}/`;
  try {
    const headers = {
      "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
      ...
      "Referer": "https://erothots1.com/free-porn",
      // Ensure this matches the browsing path
      "Origin": "https://erothots1.com",
    ...
    const html3 = await response.text();
    const { videos, hasNextPage } = extractVideoData3(html3);
    return new Response(
      JSON.stringify({ videos, nextPage: hasNextPage }),
      {
        status: 200,
        headers: {
          "Content-Type": "application/json",
...
function extractVideoData3(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const videoElements = $2("a.video, a.video.video-media");
  const videos = videoElements.map((_, video) => {
    const element = $2(video);
    const title = element.find("h3.title").text().trim();
    const time = element.find(".thumb .caption").text().trim();
    const preview = element.find(".thumb img.lazy").attr("data-src") || element.find(".thumb img.lazy").attr("src");
    const href = element.attr("href");
    const videoUrl = element.find(".thumb .player").attr("data-video");
    return { title, time, preview, href, videoUrl };
  }).get();
  const hasNextPage = !!$2("#loadmore").length;
  return { videos, hasNextPage };
}
__name(extractVideoData3, "extractVideoData3");
var init_getFree = __esm({
  "api/erothots/getFree.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet3, "onRequestGet");
    __name2(extractVideoData3, "extractVideoData");
  }
});
async function onRequestGet4({ request }) {
  const { searchParams } = new URL(request.url);
  const page = searchParams.get("p") || 0;
  const targetURL = `https://erothots1.com/videos/hot?p=${page}/`;
  try {
    const headers = {
      "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
      "Accept-Encoding": "gzip, deflate, br",
      "Accept-Language": "en-US,en;q=0.9",
      "Cache-Control": "no-cache",
      "Referer": "https://erothots1.com/videos/hot",
      // Ensure this matches the browsing path
      "Origin": "https://erothots1.com",
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36"
      // Match your browser
    ...
    const html3 = await response.text();
    const { videos, hasNextPage } = extractVideoData4(html3);
    return new Response(
      JSON.stringify({ videos, nextPage: hasNextPage }),
      {
        status: 200,
...
function extractVideoData4(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const videoElements = $2("a.video, a.video.video-media");
  const videos = videoElements.map((_, video) => {
...
__name(extractVideoData4, "extractVideoData4");
var init_getHot = __esm({
  "api/erothots/getHot.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet4, "onRequestGet");
    __name2(extractVideoData4, "extractVideoData");
  }
});
async function onRequestGet5({ request }) {
  const { searchParams } = new URL(request.url);
  const page = searchParams.get("p") || 0;
  const targetURL = `https://erothots1.com/videos/new?p=${page}/`;
  try {
    const headers = {
      "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
      "Accept-Encoding": "gzip, deflate, br",
      "Accept-Language": "en-US,en;q=0.9",
      "Cache-Control": "no-cache",
      "Referer": "https://erothots1.com/videos/new",
      // Ensure this matches the browsing path
      "Origin": "https://erothots1.com",
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36"
      // Match your browser
    ...
    const html3 = await response.text();
    const { videos, hasNextPage } = extractVideoData5(html3);
    return new Response(
      JSON.stringify({ videos, nextPage: hasNextPage }),
      {
        status: 200,
...
function extractVideoData5(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const videoElements = $2("a.video, a.video.video-media");
  const videos = videoElements.map((_, video) => {
...
__name(extractVideoData5, "extractVideoData5");
var init_getNew = __esm({
  "api/erothots/getNew.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet5, "onRequestGet");
    __name2(extractVideoData5, "extractVideoData");
  }
});
async function onRequestGet6({ request }) {
  ...
  const targetURL = `https://erothots1.com/videos/${query}?p=${page}/`;
  try {
    const headers = {
      "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
      "Accept-Encoding": "gzip, deflate, br",
      "Accept-Language": "en-US,en;q=0.9",
      "Cache-Control": "no-cache",
      "Referer": "https://erothots1.com/videos/",
      // Ensure this matches the browsing path
      "Origin": "https://erothots1.com",
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36"
      // Match your browser
    ...
    const html3 = await response.text();
    const { videos, hasNextPage } = extractVideoData6(html3);
    return new Response(
      JSON.stringify({ videos, nextPage: hasNextPage }),
      {
        status: 200,
...
function extractVideoData6(html3) {
  const cheerio = (init_browser(), __toCommonJS(browser_exports));
  const $2 = cheerio.load(html3);
  const videoElements = $2("a.video, a.video.video-media");
  const videos = videoElements.map((_, video) => {
...
__name(extractVideoData6, "extractVideoData6");
var init_getSearch = __esm({
  "api/erothots/getSearch.js"() {
    init_functionsRoutes_0_11914055566704662();
    init_browser();
    __name2(onRequestGet6, "onRequestGet");
    __name2(extractVideoData6, "extractVideoData");
  }
});
async function onRequestGet7({ request }) {
  const { searchParams } = new URL(request.url);
  const videoUrl = searchParams.get("videoUrl");
  if (!videoUrl) {
    return new Response(
...
function convertToEmbedUrl(url) {
  try {
    ...
    if (pathSegments.length >= 3) {
      return `${urlParts.origin}/embed/video/${pathSegments[2]}`;
    }
    return null;
  } catch {
    return null;
...
__name(convertToEmbedUrl, "convertToEmbedUrl");
var init_getVideo2 = __esm({
  "api/erothots/getVideo.js"() {
    init_functionsRoutes_0_11914055566704662();
    __name2(onRequestGet7, "onRequestGet");
    __name2(convertToEmbedUrl, "convertToEmbedUrl");
  }
});
async function onRequestGet8({ request }) {
  const { searchParams } = new URL(request.url);
  const videoUrl = searchParams.get("videoUrl");
  if (!videoUrl) {
    return new Response(
...
var init_getVideo3 = __esm({
  "api/dramacool/getVideo.js"() {
    init_functionsRoutes_0_11914055566704662();
...
var routes;
var init_functionsRoutes_0_11914055566704662 = __esm({
  "../.wrangler/tmp/pages-mK4XoS/functionsRoutes-0.11914055566704662.mjs"() {
    init_getVideo();
    init_scrapeDESI();
    init_getFree();
    init_getHot();
...
// .wrangler/tmp/bundle-T17dsI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-T17dsI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2578679781254132.js.map
