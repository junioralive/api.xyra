...
// .wrangler/tmp/pages-BkOBkW/functionsWorker-0.04217330576400502.mjs
...
var init_esm = __esm({
  "../node_modules/domelementtype/lib/esm/index.js"() {
    init_functionsRoutes_0_374904302329643();
...
function validateApiKey2(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
...
async function onRequest8({ request, env }) {
  try {
    ...
    if (!apiKey || !validateApiKey2(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
async function onRequest10() {
  try {
    ...
    $2(".popular-mob .widget-list li").each((_, element) => {
      ...
      const link = $2(element).find("h3 a").attr("href");
      const cleanLink = link.replace(/-episode-\d+/i, "");
...
function validateApiKey3(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
...
async function onRequest11({ request, env }) {
  try {
    ...
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey3(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
function validateApiKey4(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
__name(validateApiKey4, "validateApiKey4");
async function onRequest12({ request, env }) {
  try {
    const url = new URL(request.url);
    const page = url.searchParams.get("page") || "1";
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey4(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
function validateApiKey5(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
__name(validateApiKey5, "validateApiKey5");
async function onRequest13({ request, env }) {
  try {
    const url = new URL(request.url);
    const page = url.searchParams.get("page") || "1";
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey5(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
function validateApiKey6(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
__name(validateApiKey6, "validateApiKey6");
async function onRequest14({ request, env }) {
  try {
    const url = new URL(request.url);
    const page = url.searchParams.get("page") || "1";
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey6(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
function validateApiKey7(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
...
async function onRequest15({ request, env }) {
  try {
    ...
    const page = url.searchParams.get("page") || "1";
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey7(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
    ...
    if (!query) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Query parameter is mandatory. Please provide a valid query."
        }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
...
function validateApiKey8(apiKey, env) {
  const validApiKeys = (env.API_KEYS || "").split(",");
  return validApiKeys.includes(apiKey);
}
__name(validateApiKey8, "validateApiKey8");
async function onRequest16({ request, env }) {
  try {
    const url = new URL(request.url);
    const page = url.searchParams.get("page") || "1";
    const apiKey = url.searchParams.get("api_key");
    if (!apiKey || !validateApiKey8(apiKey, env)) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Invalid or missing API key. Please provide a valid API key."
...
var routes;
var init_functionsRoutes_0_374904302329643 = __esm({
  "../.wrangler/tmp/pages-BkOBkW/functionsRoutes-0.374904302329643.mjs"() {
    init_getStream();
    init_doc();
    init_getm3u8();
    init_getVideo();
...
// .wrangler/tmp/bundle-KR5hKz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KR5hKz/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.04217330576400502.js.map
